...
| Source | Category | Refresh | Every | Key | Coverage | Limits | Fallback |
|---|---|---|---|---|---|---|---|
| **Company news: Google News (local edition) + GDELT + NewsAPI** (`company_news`) | news | incremental (since cursor) | 24 h | none | global | GDELT: 1 request / 5 s (throttled, retried on 429). NewsAPI: 100/day, dev-only, 24 h delay, 1 month history. | Google News RSS is the primary source; GDELT and NewsAPI are supplements. |
